const ASA={
 ...
 money(n){return '৳ '+Number(n||0).toLocaleString('en-BD')},
  ...
  applyCoupon(code,subtotal){const c=String(code||'').trim().toUpperCase();const coupons={WELCOME10:{type:'percent',value:10,min:500,active:true},SAVE100:{type:'flat',value:100,min:1000,active:true},FREESHIP:{type:'shipping',value:1,min:800,active:true}};const x=coupons[c];if(!x||x.active===false||subtotal<x.min)return null;const discount=x.type==='percent'?Math.round(subtotal*x.value/100):x.type==='flat'?x.value:0;const out={code:c,discount,freeShipping:x.type==='shipping'};localStorage.setItem('asa_coupon',JSON.stringify(out));return out},
  async applyCouponLive(code,subtotal){try{const {db}=await import('./firebase-config.js');const {doc,getDoc}=await import('https://www.gstatic.com/firebasejs/12.19.0/firebase-firestore.js');const c=String(code||'').trim().toUpperCase();const snap=await getDoc(doc(db,'coupons',c));if(snap.exists()){const x=snap.data();if(x.active!==false&&subtotal>=Number(x.min||0)){const discount=x.type==='percent'?Math.round(subtotal*Number(x.value||0)/100):x.type==='flat'?Number(x.value||0):0;const out={code:c,discount,freeShipping:x.type==='shipping'};localStorage.setItem('asa_coupon',JSON.stringify(out));return out}}}catch(e){}return this.applyCoupon(code,subtotal)},
